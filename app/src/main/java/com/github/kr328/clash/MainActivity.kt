...
class MainActivity : BaseActivity<MainDesign>() {
    ...
    private suspend fun MainDesign.fetch() {
        ...
        val activeProfileName = withProfile { queryActive()?.name }
        val selectedLine = withClash {
            val groups = queryProxyGroupNames(uiStore.proxyExcludeNotSelectable)
            val preferredGroup = groups.firstOrNull { it !in setOf("DIRECT", "REJECT") }

            preferredGroup?.let { groupName ->
                queryProxyGroup(groupName, uiStore.proxySort).now
                    ?.takeIf { it.isNotBlank() }
            }
        }

        setProfileName(selectedLine ?: activeProfileName)
    }
